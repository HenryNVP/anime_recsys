...
def _iterative_filter(ratings: pd.DataFrame, min_user_inter: int, min_item_inter: int) -> pd.DataFrame:
    """
    Iteratively filter users/items until all users have >= min_user_inter
    and all items have >= min_item_inter.
    """
    prev_n = -1
    cur = ratings
    while True:
        ...
        cur = cur[
            cur["user_id"].isin(uc[uc >= min_user_inter].index) &
            cur["anime_id"].isin(ic[ic >= min_item_inter].index)
        ].copy()
    return cur
...
def main():
    ...
    # ---------- Filter sparsity (iterative) ----------
    before = len(ratings)
    ratings = _iterative_filter(ratings, args.min_user_inter, args.min_item_inter)
    after = len(ratings)
    ...
    stats = {
        "raw_interactions": before, "clean_interactions": after,
    ...
    # ---------- Summary ----------
    print("\n=== Preprocess Summary ===")
    print(f"Raw ratings:      {before:,}")
    print(f"Clean ratings:    {after:,}")
